...
export default function LoadingContent() {
  ...
  useEffect(() => {
    ...
    const initParams = () => {
      ...
      console.log("LoadingContent - Final params:", { finalSubject, finalType })
  ...
  // Effet pour animer la progression - légèrement plus lent
  useEffect(() => {
    if (!initialized) return

    const interval = setInterval(() => {
      setProgress((prev) => {
        // Augmenter progressivement jusqu'à 100% à vitesse modérée
        ...
        if (newProgress >= 100) {
          ...
          setTimeout(() => {
            console.log("Redirecting with params:", urlParams)

            // Rediriger vers la page de résultats en conservant les paramètres
            router.push(`/quiz/resultat?subject=${urlParams.subject}&type=${urlParams.type}`)
          }, 300)
          return 100
        }
    ...
    }, 200) // Intervalle modéré

    return () => clearInterval(interval)
  }, [router, urlParams, initialized])

  // Effet pour faire défiler les témoignages avec animation simple
  useEffect(() => {
    const testimonialInterval = setInterval(() => {
      setCurrentTestimonialIndex((prev) => (prev === testimonials.length - 1 ? 0 : prev + 1))
    }, 4000) // Intervalle entre les témoignages
  ...
  return (
    <div className="flex flex-col items-center justify-center px-4 py-8 min-h-screen">
      <div className="w-full max-w-md mx-auto text-center">
        ...
        {/* Carrousel de témoignages avec animation simple de fondu */}
        <div className="bg-white rounded-lg p-4 shadow-lg max-w-md mx-auto transition-opacity duration-300">
